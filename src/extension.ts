    ...
    const formEventsProvider = vscode.languages.registerCompletionItemProvider(
        { language: 'javascript', scheme: 'file' },
        {
            provideCompletionItems(document, position) {
                ...
                if (!line.includes("frappe.ui.form.on") && !linePrefix.match(/frappe\.ui\.form\.on\([^)]*\)\s*\{\s*$/)) {
                    return undefined;
                }
                ...
                return formEvents.map(ev => {
                    ...
                    item.insertText = new vscode.SnippetString(`${ev.label}: function(frm) {\n\t$0\n}`);
                    item.documentation = new vscode.MarkdownString(`**${ev.label}**\n\n${ev.detail}`);
                    item.sortText = '0' + ev.label; 
                    return item;
                });
            }
        },
    ...
    const globalsProvider = vscode.languages.registerCompletionItemProvider(
        { language: 'javascript', scheme: 'file' },
        {
            provideCompletionItems(document, position) {
                ...
                if (!/\s$|^$/.test(linePrefix.charAt(linePrefix.length - 1))) {
                    return undefined;
                }

                const globals = [
                   
                    { label: 'cur_frm', kind: vscode.CompletionItemKind.Variable, detail: 'Current form object', insertText: 'cur_frm' },
                    { label: 'cur_list', kind: vscode.CompletionItemKind.Variable, detail: 'Current list object', insertText: 'cur_list' },
                    { label: 'cur_dialog', kind: vscode.CompletionItemKind.Variable, detail: 'Current open dialog', insertText: 'cur_dialog' },
                    { label: 'cur_page', kind: vscode.CompletionItemKind.Variable, detail: 'Current page object', insertText: 'cur_page' },
                    { label: 'locals', kind: vscode.CompletionItemKind.Variable, detail: 'All documents and DocTypes in session', insertText: 'locals' },
                    { label: 'frappe', kind: vscode.CompletionItemKind.Variable, detail: 'Main Frappe object', insertText: 'frappe' },
                ];

                return globals.map(global => {
                    const item = new vscode.CompletionItem(global.label, global.kind);
                    item.detail = global.detail;
                    item.insertText = global.insertText;
                    item.documentation = new vscode.MarkdownString(global.detail);
                    item.sortText = '0' + global.label; 
                    return item;
                });
            }
        },
